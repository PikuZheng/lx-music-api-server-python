...
def signRequest(url, params, options):
    ...
    url = (
        ...
        + sign(
            ...
            options.get("body")
            if options.get("body")
            else (options.get("data") if options.get("data") else ""),
        )
    )
...
def getKey(hash_):
    return utils.createMD5(
        hash_.lower() + tools.pidversec + tools.appid + tools.mid + tools.userid
    )
...
async def url(songId, quality):
    ...
    albumid = (
        body_["album_info"]["album_id"]
        if (body_.get("album_info") and body_["album_info"].get("album_id"))
        else None
    )
    ...
    return {"url": body.url[0], "quality": quality}
